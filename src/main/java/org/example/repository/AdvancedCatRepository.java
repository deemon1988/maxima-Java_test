   ...
   /* String propertiesPath = "application.properties";
    ...
    public int update(Long id, Cat element) {
        try {
            ...
            // ResultSet result = preStatement.executeQuery("UPDATE cats SET name=?, weight=?, isAngry=?  WHERE id = ?");
           ...
           /* String query = (String.format("UPDATE cats SET name=? WHERE id='%d'",id));
    ...
    public void delete(Long id) {
        try {
           ...
           /* Statement statement = connection.createStatement();
            int rows = statement.executeUpdate("DELETE FROM cats WHERE id=%d");
            System.out.println("Удалено записей: " + rows);*/
            connection.close();
        } catch (SQLException e) {
            // throw new RuntimeException(e);
